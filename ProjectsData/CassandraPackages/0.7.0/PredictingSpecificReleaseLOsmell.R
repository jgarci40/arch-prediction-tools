...
{  
	model.glm.nb <- glm.nb(BUOnextRelease ~ log2(CountClassCoupled + 1) + BUO + log2(internalEdges + 
...
{
	model.lm <- lm(BUOnextRelease ~ MaxInheritanceTree + coChangedDifferentPackage + 
    coChangedSamePackage + BUO + incomingDep + internalEdges + 
