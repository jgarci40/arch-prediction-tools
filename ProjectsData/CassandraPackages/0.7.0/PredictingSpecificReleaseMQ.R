...
{  
		model.glm.nb <- glm.nb(MQnextRelease ~ log2(LOC+1) + log2(numberOfCommits+1) + log2(CountClassCoupled+1) + log2(MaxInheritanceTree+1) + log2(PercentLackOfCohesion+1) + log2(SumCyclomatic+1) +log2(coChangedDifferentPackage+1)+log2(coChangedSamePackage+1)+ log2(NumCochangedFiles+1) + BDC + BUO +log2(incomingDep+1) + log2(outgoingDep+1) + log2(internalEdges +1) + log2(externalEdges +1) + log2(edgesInto+1) + log2(edgesOutOf+1), data=train)
...
{
	model.lm <- lm(MQnextRelease ~ LOC + CountClassCoupled + MaxInheritanceTree + 
    NumCochangedFiles + coChangedDifferentPackage + coChangedSamePackage + 
    BDC + BUO + internalEdges + externalEdges + edgesInto, data=train)
