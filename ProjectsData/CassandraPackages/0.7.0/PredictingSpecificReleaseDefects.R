...
{  
	model.glm.nb <- glm.nb(BugCountMult ~ log2(LOC + 1) + log2(MaxInheritanceTree + 1) + 
    log2(NumCochangedFiles + 1) + BUO + log2(internalEdges + 
...
{
	model.lm <- lm(BugCountMult ~ MaxInheritanceTree + NumCochangedFiles + coChangedDifferentPackage + 
    coChangedSamePackage + BUO + internalEdges + edgesInto, data=train)
