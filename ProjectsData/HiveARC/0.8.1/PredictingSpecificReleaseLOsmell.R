...
{  
	model.glm.nb <- glm.nb(BUOnextRelease ~ BCO + BUO + log2(incomingDep + 1) + log2(externalEdges + 
    1) + log2(edgesInto + 1) + log2(edgesOutOf + 1), data=train)	
...
{
	model.lm <- lm(BUOnextRelease ~ LOC + CountClassCoupled + SumCyclomatic + NumCochangedFiles + 
    coChangedDifferentPackage + BCO + BUO + incomingDep + edgesInto, data=train)
