...
{  
	model.glm.nb <- glm.nb(newSPF ~ log2(MaxInheritanceTree + 1) + SPF
, data=train)	
...
{
	model.lm <- lm(newSPF ~ numberOfCommits + NumCochangedFiles + coChangedSamePackage + 
    BDC + internalEdges + externalEdges + edgesInto, data=train)
