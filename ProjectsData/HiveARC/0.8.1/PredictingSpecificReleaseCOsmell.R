...
{  
	model.glm.nb <- glm.nb(BCOnextRelease ~ log2(coChangedSamePackage + 1) + BCO + BUO, data=train)	
...
{
	model.lm <- lm(BCOnextRelease ~ MaxInheritanceTree + PercentLackOfCohesion + 
    NumCochangedFiles + coChangedDifferentPackage + BCO + BDC, data=train)
