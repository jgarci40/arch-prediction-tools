...
{  
	model.glm.nb <- glm.nb(BugCountMult ~ log2(MaxInheritanceTree + 1) + log2(coChangedDifferentPackage + 
    1) + log2(coChangedSamePackage + 1) + log2(NumCochangedFiles + 
    1) + BCO + log2(edgesInto + 1) + log2(CountClassCoupled + 
    1), data=train)	
...
classification_linear <- function (train, test) 
{
    ...
    NumCochangedFiles + coChangedSamePackage + BDC + incomingDep + 
    outgoingDep + externalEdges, data=train)
