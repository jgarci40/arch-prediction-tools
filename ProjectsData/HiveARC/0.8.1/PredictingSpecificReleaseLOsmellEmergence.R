...
{  
    ...
    1) + BCO + BUO + log2(incomingDep + 1) + log2(edgesInto + 
    1)
, data=train)	
...
{
	model.lm <- lm(newBUO ~ LOC + numberOfCommits + SumCyclomatic + NumCochangedFiles + 
    BCO + BUO + incomingDep + outgoingDep + edgesInto, data=train)
