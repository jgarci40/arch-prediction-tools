...
{  
	model.glm.nb <- glm.nb(SPFnextRelease ~ log2(coChangedSamePackage + 1) + SPF + BUO, data=train)	
...
{
	model.lm <- lm(SPFnextRelease ~ numberOfCommits + CountClassCoupled + coChangedSamePackage + 
    SPF + BUO + edgesInto, data=train)
