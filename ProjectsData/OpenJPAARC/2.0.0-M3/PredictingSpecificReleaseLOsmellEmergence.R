...
{  
	model.glm.nb <- glm.nb(newBUO ~ BUO + log2(outgoingDep + 1) + log2(edgesInto + 1)
, data=train)	
...
{
	model.lm <- lm(newBUO ~ CountClassCoupled + coChangedDifferentPackage + BUO + 
    incomingDep + outgoingDep + externalEdges, data=train)
