...
{  
	model.glm.nb <- glm.nb(BugCountMult ~ log2(LOC + 1) + log2(numberOfCommits + 1) + log2(coChangedDifferentPackage + 
    1) + log2(coChangedSamePackage + 1) + BDC + log2(incomingDep + 
    1) + log2(outgoingDep + 1) + log2(internalEdges + 1) + log2(externalEdges + 
    1) + log2(edgesInto + 1), data=train)	
...
{
	model.lm <- lm(BugCountMult ~ numberOfCommits + CountClassCoupled + SumCyclomatic + 
    NumCochangedFiles + coChangedDifferentPackage + coChangedSamePackage + 
    internalEdges, data=train)
