...
{  
	model.glm.nb <- glm.nb(BugCountMult ~ log2(LOC + 1) + log2(numberOfCommits + 1) + log2(coChangedSamePackage + 
    1) + log2(outgoingDep + 1) + log2(internalEdges + 1) + log2(edgesOutOf + 
    1), data=train)	
...
{
	model.lm <- lm(BugCountMult ~ LOC + numberOfCommits + SumCyclomatic + coChangedSamePackage, data=train)
