...
{  
	model.glm.nb <- glm.nb(BUOnextRelease ~ BDC + BUO + log2(edgesInto + 1) + log2(edgesOutOf + 
    1), data=train)	
...
classification_linear <- function (train, test) 
{
    ...
    PercentLackOfCohesion + NumCochangedFiles + coChangedSamePackage + 
    BDC + BUO + incomingDep + outgoingDep + externalEdges + edgesInto, data=train)
