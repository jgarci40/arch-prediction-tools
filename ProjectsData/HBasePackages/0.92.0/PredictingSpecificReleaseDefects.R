...
{  
    ...
    1) + BDC + log2(incomingDep + 1) + log2(outgoingDep + 1), data=train)	
...
{
	model.lm <- lm(BugCountMult ~ LOC + numberOfCommits + CountClassCoupled + NumCochangedFiles + 
    coChangedSamePackage + outgoingDep + edgesInto, data=train)
