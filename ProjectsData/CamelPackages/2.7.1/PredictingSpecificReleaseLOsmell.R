...
{  
	model.glm.nb <- glm.nb(BUOnextRelease ~ BUO + log2(outgoingDep + 1), data=train)	
...
{
	model.lm <- lm(BUOnextRelease ~ coChangedDifferentPackage + coChangedSamePackage + 
    BUO + outgoingDep + internalEdges + externalEdges + edgesInto, data=train)
