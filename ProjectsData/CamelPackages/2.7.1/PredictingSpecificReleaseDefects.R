...
{  
    ...
    1) + log2(MaxInheritanceTree + 1) + log2(coChangedSamePackage + 
    1) + log2(NumCochangedFiles + 1) + BDC + log2(outgoingDep + 
    1), data=train)	
...
classification_linear <- function (train, test) 
{
    ...
    BDC + incomingDep + internalEdges + externalEdges + edgesInto, data=train)
