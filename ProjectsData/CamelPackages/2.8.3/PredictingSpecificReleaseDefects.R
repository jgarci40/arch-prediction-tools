...
{  
	model.glm.nb <- glm.nb(BugCountMult ~ log2(LOC + 1) + log2(numberOfCommits + 1) + log2(CountClassCoupled + 
    1) + log2(MaxInheritanceTree + 1) + log2(coChangedDifferentPackage + 
    1) + log2(coChangedSamePackage + 1) + log2(NumCochangedFiles + 
    1) + BDC + log2(outgoingDep + 1), data=train)	
...
{
	model.lm <- lm(BugCountMult ~ LOC + numberOfCommits + CountClassCoupled + MaxInheritanceTree + 
    SumCyclomatic + NumCochangedFiles + coChangedDifferentPackage + 
    coChangedSamePackage + incomingDep + outgoingDep + internalEdges, data=train)
