...
{  
	model.glm.nb <- glm.nb(BUOnextRelease ~ log2(numberOfCommits + 1) + log2(coChangedSamePackage + 
    1) + log2(NumCochangedFiles + 1) + BDC + BUO + log2(externalEdges + 
...
{
	model.lm <- lm(BUOnextRelease ~ LOC + coChangedDifferentPackage + BDC + BUO + 
