...
{  
	model.glm.nb <- glm.nb(BugCountMult ~ log2(CountClassCoupled + 1) + log2(SumCyclomatic + 
...
{
	model.lm <- lm(BugCountMult ~ CountClassCoupled + MaxInheritanceTree + NumCochangedFiles + 
    coChangedDifferentPackage + coChangedSamePackage + externalEdges, data=train)
