...
{  
	model.glm.nb <- glm.nb(MQnextRelease ~ log2(internalEdges + 1), data=train)	
...
{
	model.lm <- lm(MQnextRelease ~ LOC + CountClassCoupled + PercentLackOfCohesion + 
    NumCochangedFiles + coChangedDifferentPackage + BDC + incomingDep + 
