...
{  
	model.glm.nb <- glm.nb(newBUO ~ log2(numberOfCommits + 1) + log2(CountClassCoupled + 
    1) + log2(SumCyclomatic + 1) + log2(coChangedSamePackage + 
    1) + BUO + log2(incomingDep + 1) + log2(outgoingDep + 1) + 
    log2(internalEdges + 1) + log2(edgesOutOf + 1)
, data=train)	
...
{
	model.lm <- lm(newBUO ~ numberOfCommits + coChangedSamePackage + BDC + BUO + 
    outgoingDep + internalEdges + externalEdges + edgesInto, data=train)
