...
{  
	model.glm.nb <- glm.nb(BUOnextRelease ~ log2(LOC + 1) + log2(CountClassCoupled + 1) + 
    log2(PercentLackOfCohesion + 1) + BCO + log2(externalEdges + 
    1), data=train)	
...
{
	model.lm <- lm(BUOnextRelease ~ numberOfCommits + MaxInheritanceTree + PercentLackOfCohesion + 
    coChangedDifferentPackage + BCO + BUO + incomingDep + outgoingDep, data=train)
