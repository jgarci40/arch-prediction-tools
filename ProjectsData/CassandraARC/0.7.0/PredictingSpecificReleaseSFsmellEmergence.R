...
{  
	model.glm.nb <- glm.nb(newSPF ~ log2(numberOfCommits + 1) + log2(CountClassCoupled + 
    1) + log2(PercentLackOfCohesion + 1) + log2(SumCyclomatic + 
    1) + log2(coChangedSamePackage + 1) + log2(NumCochangedFiles + 
    1) + SPF + BUO
, data=train)	
...
{
	model.lm <- lm(newSPF ~ LOC + numberOfCommits + CountClassCoupled + SumCyclomatic + 
