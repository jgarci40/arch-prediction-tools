...
{
	model.lm <- lm(MQnextRelease ~ LOC + numberOfCommits + CountClassCoupled + SumCyclomatic + 
    coChangedDifferentPackage + SPF + BUO + outgoingDep + internalEdges + 
...
{
	randomForest <- randomForest(MQnextRelease ~ CountClassCoupled + SumCyclomatic + SPF + BUO + 
    incomingDep + outgoingDep + internalEdges + externalEdges + 
