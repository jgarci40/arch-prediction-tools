...
{  
	model.glm.nb <- glm.nb(BUOnextRelease ~ log2(LOC + 1) + BCO + log2(outgoingDep + 1) + 
    log2(externalEdges + 1), data=train)	
...
{
	model.lm <- lm(BUOnextRelease ~ CountClassCoupled + SumCyclomatic + coChangedDifferentPackage + 
    BCO + incomingDep + outgoingDep, data=train)
