...
{  
	model.glm.nb <- glm.nb(BCOnextRelease ~ log2(LOC + 1) + log2(CountClassCoupled + 1) + 
    log2(PercentLackOfCohesion + 1) + log2(coChangedSamePackage + 
    1) + BCO + BDC + log2(outgoingDep + 1) + log2(edgesOutOf + 
    1), data=train)	
...
{
	model.lm <- lm(BCOnextRelease ~ PercentLackOfCohesion + BCO, data=train)
