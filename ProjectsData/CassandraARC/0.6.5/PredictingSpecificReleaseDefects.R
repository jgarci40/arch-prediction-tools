...
{  
	model.glm.nb <- glm.nb(BugCountMult ~ log2(numberOfCommits + 1) + log2(CountClassCoupled + 
    1) + log2(coChangedSamePackage + 1) + log2(NumCochangedFiles + 
    1) + BUO + log2(internalEdges + 1) + log2(externalEdges + 
    1), data=train)	
...
classification_linear <- function (train, test) 
{
    ...
    BUO + incomingDep + outgoingDep, data=train)
