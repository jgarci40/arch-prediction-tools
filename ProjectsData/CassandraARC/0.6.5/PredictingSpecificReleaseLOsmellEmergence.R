...
{  
	model.glm.nb <- glm.nb(newBUO ~ log2(LOC + 1) + log2(CountClassCoupled + 1) + BCO + 
    BUO + log2(incomingDep + 1) + log2(externalEdges + 1) + log2(edgesInto + 
    1)
, data=train)	
...
{
	model.lm <- lm(newBUO ~ CountClassCoupled + SumCyclomatic + coChangedDifferentPackage + 
    BCO + BUO + incomingDep + outgoingDep, data=train)
