...
{  
	model.glm.nb <- glm.nb(newBUO ~ log2(CountClassCoupled + 1) + log2(MaxInheritanceTree + 
    1) + log2(coChangedSamePackage + 1) + BDC + BUO + log2(incomingDep + 
    1) + log2(outgoingDep + 1) + log2(externalEdges + 1)
, data=train)	
...
summary(m1 <- lm(newBUO ~ LOC + numberOfCommits + CountClassCoupled + MaxInheritanceTree + PercentLackOfCohesion + SumCyclomatic + NumCochangedFiles + coChangedDifferentPackage + coChangedSamePackage + BDC + BUO + incomingDep + outgoingDep + internalEdges + externalEdges + edgesInto + edgesOutOf, data= TrainingData))
...
{
	model.lm <- lm(newBUO ~ BUO + incomingDep + outgoingDep + internalEdges + externalEdges + 
...
{
	randomForest <- randomForest(newBUO ~ LOC + numberOfCommits + CountClassCoupled + MaxInheritanceTree + PercentLackOfCohesion + SumCyclomatic + NumCochangedFiles + coChangedDifferentPackage + coChangedSamePackage + BDC + BUO + incomingDep + outgoingDep + internalEdges + externalEdges + edgesInto + edgesOutOf, data= train)
