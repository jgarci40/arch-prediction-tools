...
{  
    ...
    1) + BCO + SPF + BUO + log2(incomingDep + 1) + log2(internalEdges + 
    1) + log2(edgesOutOf + 1), data=train)	
...
{
	model.lm <- lm(BUOnextRelease ~ LOC + CountClassCoupled + MaxInheritanceTree + 
    SumCyclomatic + coChangedDifferentPackage + coChangedSamePackage + 
    BCO + BDC + BUO + incomingDep + internalEdges + edgesInto, data=train)
