...
{  
	model.glm.nb <- glm.nb(newBUO ~ log2(LOC + 1) + log2(MaxInheritanceTree + 1) + log2(coChangedDifferentPackage + 
    1) + BUO + log2(incomingDep + 1) + log2(externalEdges + 1)
, data=train)	
...
{
	model.lm <- lm(newBUO ~ LOC + CountClassCoupled + MaxInheritanceTree + PercentLackOfCohesion + 
    SumCyclomatic + coChangedDifferentPackage + coChangedSamePackage + 
    BDC + BUO + incomingDep + internalEdges + edgesInto, data=train)
