...
{  
	model.glm.nb <- glm.nb(MQnextRelease ~ log2(internalEdges + 1) + log2(edgesInto + 1), data=train)		
...
{
	model.lm <- lm(MQnextRelease ~ CountClassCoupled + PercentLackOfCohesion + coChangedDifferentPackage + 
    coChangedSamePackage + BUO + incomingDep + outgoingDep + 
    internalEdges + externalEdges + edgesInto, data=train)
...
{
	randomForest <- randomForest(MQnextRelease ~ LOC + numberOfCommits + CountClassCoupled + MaxInheritanceTree + PercentLackOfCohesion + SumCyclomatic + NumCochangedFiles + coChangedDifferentPackage + coChangedSamePackage + BDC + BUO + incomingDep + outgoingDep + internalEdges + externalEdges + edgesInto + edgesOutOf, data= train)
