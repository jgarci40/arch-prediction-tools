...
{  
    ...
    log2(coChangedSamePackage + 1) + log2(NumCochangedFiles + 
    1) + SPF + BDC + BUO + log2(incomingDep + 1) + log2(outgoingDep + 
    1) + log2(internalEdges + 1) + log2(externalEdges + 1), data=train)	
...
{
	model.lm <- lm(BUOnextRelease ~ PercentLackOfCohesion + BDC + BUO + outgoingDep, data=train)
