...
{  
	model.glm.nb <- glm.nb(newBDC ~ BDC + log2(incomingDep + 1) + log2(outgoingDep + 1) + 
...
{
	model.lm <- lm(newBDC ~ SumCyclomatic + coChangedDifferentPackage + coChangedSamePackage + 
    BCO + BDC + incomingDep + edgesInto, data=train)
