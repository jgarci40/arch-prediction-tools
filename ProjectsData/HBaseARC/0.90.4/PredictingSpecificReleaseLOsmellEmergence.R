...
{  
	model.glm.nb <- glm.nb(newBUO ~ log2(numberOfCommits + 1) + log2(coChangedSamePackage + 
    1) + log2(NumCochangedFiles + 1) + SPF + BDC + BUO + log2(outgoingDep + 
    1) + log2(internalEdges + 1) + log2(externalEdges + 1) + 
    log2(edgesInto + 1) + log2(edgesOutOf + 1)
, data=train)	
...
{
	model.lm <- lm(newBUO ~ MaxInheritanceTree + NumCochangedFiles + BCO + BDC + 
    incomingDep + edgesInto, data=train)
