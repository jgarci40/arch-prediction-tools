...
{  
    ...
    1) + SPF + BDC + BUO + log2(outgoingDep + 1) + log2(internalEdges + 
    1) + log2(externalEdges + 1) + log2(edgesInto + 1), data=train)	
...
{
	model.lm <- lm(BUOnextRelease ~ PercentLackOfCohesion + BCO + BDC + BUO, data=train)
